package Validation.EmailValidation;
...
public class Model{
	...
	private static boolean running;						// The flag that specifies if the FSM is 
														// running
	private final static int maxDomainCounter = 63;		// Max Domain Counter
	...
	public static String checkEmailAddress(String input) {
		...
		while (running) {
			...
			case 1: 
				...
				//State 1 receive the Local Part move back to State 1
				if ((currentChar >= 'A' && currentChar <= 'Z')|| 		// Upper case
						(currentChar >= 'a' && currentChar <= 'z') ||	// Lower case
						(currentChar >= '0' && currentChar <= '9')) {	// Digit
					nextState = 1;
				}
				// State 1 receive @ move to State 2
				else if (currentChar == '@'){
					nextState = 2;
				}
				// State 1 receive . move to State 0
				else if (currentChar == '.') {
					nextState = 1;
				}
				// State 1 return an error
				else {
					running = false;
				}
			...
			case 2: 
				...
				if ((currentChar >= 'A' && currentChar <= 'Z')|| 		// Upper case
						(currentChar >= 'a' && currentChar <= 'z') ||	// Lower case
						(currentChar >= '0' && currentChar <= '9') &&	// Digit
						(domainPartCounter <= maxDomainCounter)) {	
					nextState = 3;
					domainPartCounter++;
				}
				else {
		...
		case 1:
			...
			emailAddressErrorMessage = "Invalid email Local Part format.\n";
			return emailAddressErrorMessage;
			// Replace this with the required code
		...
		case 2:
			...
			emailAddressIndexofError = currentCharNdx;
			emailAddressErrorMessage = "Domain name missing after @.\n";
			return emailAddressErrorMessage;		
			// Replace this with the required code
		...
		case 4:
			...
			emailAddressIndexofError = currentCharNdx;
			emailAddressErrorMessage = "Invalid use of hyphen in domain name.\n";
			return emailAddressErrorMessage;	
			// Replace this with the required code
