...
public class View {
	...
	static private Label label_Confirmed_Password = new Label("Enter the confirmed password here");
	static protected TextField text_Confirmed_Password = new PasswordField();
	...
	static protected Label err_confirmedPasswordDoesNotMatch = new Label(); 
	...
	static protected Label label_noteLogOut = 
			new Label("Note: User is required to log in after changing the password");
	...
	public static void view (Pane theRoot) {
		...
		setupLabelWidget(label_Confirmed_Password, 10, 80, "Arial", 14, windowWidth-10, Pos.BASELINE_LEFT);
		
		// Establish the text input operand field and when anything changes in the password,
		// the code will process the entire input to ensure that it is valid or in error.
		setupTextWidget(text_Confirmed_Password, 10, 100, "Arial", 18, windowWidth-20, Pos.BASELINE_LEFT, 
				true);
		
		text_Password.textProperty().addListener((observable, oldValue, newValue) 
				-> {Model.updatePassword(); });
		
		text_Confirmed_Password.textProperty().addListener((observable, oldValue, newValue) 
				-> {Model.updateConfirmedPassword();});
		
		
		// Establish an error message for when there is no input
//		setupLabelWidget(noInputFound, 10, 70, "Arial", 14, windowWidth-10, Pos.BASELINE_LEFT);	
...
//		setupLabelWidget(errPasswordPart3, 20, 110, "Arial", 14, 200, Pos.BASELINE_LEFT);
		...
		setupLabelWidget(err_confirmedPasswordDoesNotMatch, 10, 135, "Arial", 14, windowWidth - 10, Pos.BASELINE_LEFT);
		
		// Position the assessment display of the various requirements components
	    setupLabelWidget(label_Requirements, 10, 160, "Arial", 16, windowWidth-10, 
	    		Pos.BASELINE_LEFT);
	    
	    setupLabelWidget(label_UpperCase, 30, 190, "Arial", 14, windowWidth-10, 
				Pos.BASELINE_LEFT);

	    setupLabelWidget(label_LowerCase, 30, 220, "Arial", 14, windowWidth-10, 
				Pos.BASELINE_LEFT);
	    
	    setupLabelWidget(label_NumericDigit, 30, 250, "Arial", 14, windowWidth-10, 
				Pos.BASELINE_LEFT);
	    
	    setupLabelWidget(label_SpecialChar, 30, 280, "Arial", 14, windowWidth-10, 
				Pos.BASELINE_LEFT);
	    
	    setupLabelWidget(label_LongEnough, 30, 310, "Arial", 14, windowWidth-10, 
				Pos.BASELINE_LEFT);
	    
	    setupLabelWidget(label_noteLogOut, 30, 350, "Arial", 16, windowWidth - 10, Pos.BASELINE_LEFT);
...
//				label_SpecialChar, label_LongEnough, button_Finish, label_Confirmed_Password, text_Confirmed_Password);
		theRoot.getChildren().addAll(label_Password, text_Password,
				label_Requirements, label_UpperCase, label_LowerCase, label_NumericDigit,
				label_SpecialChar, label_LongEnough, button_Finish, label_Confirmed_Password, text_Confirmed_Password, 
				err_confirmedPasswordDoesNotMatch, label_noteLogOut);
	}
