...
public class Model {
	...
	protected static void updatePassword() {
		...
		else
		{
			...
			else if (!checkConfirmedPassword()) {
				View.button_Finish.setDisable(true);
			}
			else {
	...
	protected static void updateConfirmedPassword() {
		String password = View.text_Password.getText();
		String errMessage = evaluatePassword(password);

		if (!checkConfirmedPassword() || errMessage.compareTo("") != 0) {
			View.button_Finish.setDisable(true);
		}
		else {
			View.button_Finish.setDisable(false);
	...
	protected static boolean checkConfirmedPassword() {
		String confirmedPassword = View.text_Confirmed_Password.getText();
		String password = View.text_Password.getText();
		
		if (confirmedPassword.compareTo(password) != 0) {
			if (confirmedPassword.compareTo("") != 0) {
				View.err_confirmedPasswordDoesNotMatch.setText("The confirmed password does not match");
				View.err_confirmedPasswordDoesNotMatch.setTextFill(Color.RED);
			}
			
			return false;
		}
		else {
			View.err_confirmedPasswordDoesNotMatch.setText("");
			return true;
		}
	}
